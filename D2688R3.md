...
# Updates R2 → R3
  - [Require Parentheses on Match Guards]
    - `@*expr*@ match @*pattern*@ if @[(]{.add}@ @*condition*@ @[)]{.add}@`
    - ```cpp
      @*expr*@ match {
        @*pattern*@ if @[(]{.add}@ @*condition*@ @[)]{.add}@ => @*expr-stmt*@
        // ...
      };
      ```
    - Support *init-stmt* and condition variables in match guards.
...
+
+ @*match-test-expression*@
+     @*match-subject*@ match constexpr@*~opt~*@ @*match-test-pattern*@ @*match-guard~opt~*@
+
+ @*match-subject*@:
+     @*pm-expression*@
+     @*braced-init-list*@
...
Lastly, a code search for `match` on boolean value in Rust code on GitHub showed
that this use case is actually quite popular. This was a bit of a surprise,
as Rust already supports `if` expressions which could be used for the same
purpose. This finding also weakened the assumption the main argument behind
the approach.
...
Consider adding a match guard to this expression. In previous revisions
of this paper, match guards did not require parentheses.
...
This situation is considerably worse than the `match` scenario since it's much
more likely for the match guard condition to be a relatively complex boolean
expression.
