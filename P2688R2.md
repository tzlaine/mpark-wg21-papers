...
+
+ @*match-test-expression*@
+     @*match-subject*@ match constexpr@*~opt~*@  @*match-test-guard~opt~*@
+     @*match-subject*@ match constexpr@*~opt~*@  @*match-test-guard~opt~*@
+
+ @*match-subject*@:
+     @*pm-expression*@
...
+
+ @*match-select-expression*@
+     @*match-subject*@ match constexpr@*~opt~*@ @*trailing-return-type~opt~*@ { @*match-case-seq*@ }
+
+ @*match-case-seq*@:
+     @*match-case*@ @*match-case-seq~opt~*@
+
+ @*match-case*@:
+     @*match-case-pattern*@ @*match-case-guard~opt~*@ => @*expr-or-braced-init-list*@ ;
+     @*match-case-pattern*@ @*match-case-guard~opt~*@ => @*escape-statement*@ ;
+
+ @*match-case-guard*@:
...
Another advantage is that (though perhaps silly) it's typically work to add
...
The following is a rough overview of the current status:
  ...
  - **Parsing**
    - Structure
      ...
      - `constexpr`: `expr match constexpr { ... }`
      ...
      - _Missing_: Matching against _braced-init-list_ `{ a, b } match pattern`
    - Patterns
  ...
  - **Semantic Anaylsis**
    ...
    - _Missing_: Inject bindings into the enclosing control statement. e.g. `if (expr match [0, let x]) { /* x available here */ }`
  ...
  - **Code Gen**
    - Most of constant evaluation has been implemented.
      ...
      - _Missing_: For alternative pattern, `@*type-id*@: pattern`, currently it only implements polymorphic types
                   using `dynamic_cast` under the hood.
...
it folds a sequence of RHS into LHS based on the associativity and precedence of the found operator.
...
For example, `_` is a the wildcard pattern, and it is so even if there is a variable named `_` in scope.
...
It turns out, the difficulty of parsing parenthesized pattern turns out is not much harder than the difficulty
